...
# ─────────────────────────────────────────────────────────────────────────────
# CONFIG FILE  — stores hashed user credentials persistently
# On Streamlit Cloud the file lives in the repo or is bootstrapped from secrets.
# ─────────────────────────────────────────────────────────────────────────────
...
def get_authenticator():
    ...
    config = load_config()
    # v0.4.x API — no pre_authorized param in Authenticate()
...
def render_hero(r, dark):
    ...
    st.markdown(f'<div class="card"><h3 style="font-size:1.22rem;border-bottom:1px solid var(--border);padding-bottom:.82rem;margin-bottom:1.25rem">📊 Analysis Results</h3>'
                ...
                f'<p style="font-size:.74rem;font-weight:700;color:{mu};text-transform:uppercase;letter-spacing:.09em;margin-bottom:.68rem">🔢 All Predictions</p>'
...
def render_quality(q, dark):
    ...
    for l,s in q['all_scores'].items():
        ...
        bars+=(f'<div class="bar-row"><div class="bar-label">'
               f'<span style="{fw};text-transform:capitalize;font-size:.79rem">{"▶ " if w else ""}{l}</span>'
               f'<span style="{fw};font-size:.79rem">{s:.1f}%</span></div>{abar(s,col,8)}</div>')
    st.markdown(f'<div class="card" style="height:100%"><h3>🎚️ Audio Quality</h3>'
...
def render_condition(cond, dark):
    ...
    st.markdown(
        f'<div class="card" style="height:100%"><h3>⏳ Audio Condition</h3>'
        f'<p style="font-size:.66rem;font-weight:700;color:{mu};text-transform:uppercase;letter-spacing:.08em;margin-bottom:.68rem">🕰️ Recording Era</p>'
        f'<div class="bar-row"><div class="bar-label">'
        f'<span style="font-weight:700;color:{"#818cf8" if im and dark else "#2563eb" if im else mu};font-size:.79rem">💿 Modern {meb}</span>'
        ...
        f'<span style="font-weight:700;color:{"#fb923c" if not im else mu};font-size:.79rem">🕰️ Vintage {veb}</span>'
        f'<span style="font-weight:800;color:{"#fb923c" if not im else mu};font-size:.79rem">{vp:.1f}%</span></div>{abar(vp,"#fb923c",16)}</div>'
        f'<p style="font-size:.66rem;font-weight:700;color:{mu};text-transform:uppercase;letter-spacing:.08em;margin:1.1rem 0 .52rem">🔊 Signal Quality</p>'
        f'<div class="sigbox {"snoisy" if isn else "sclean"}">'
        f'<div style="display:flex;align-items:center;gap:.68rem">'
        f'<span style="font-size:1.32rem">{"📻" if isn else "✨"}</span>'
        ...
        f'</div></div><span style="font-size:1.32rem">{"⚠️" if isn else "✅"}</span></div>'
        f'<div style="padding:0 .12rem">{sub}</div></div>', unsafe_allow_html=True)

def render_info(result):
    ai=result['audio_info']
    st.markdown(f'<div class="card"><h3>ℹ️ Audio Information</h3><div class="igrid">'
                f'<div class="itile tb"><div class="t-ico">⏱️</div><div class="t-lbl">Duration</div><div class="t-val">{ai["duration"]:.2f}s</div></div>'
                f'<div class="itile tp"><div class="t-ico">〰️</div><div class="t-lbl">Sample Rate</div><div class="t-val">{ai["sample_rate"]} Hz</div></div>'
                f'<div class="itile ti"><div class="t-ico">💾</div><div class="t-lbl">Samples</div><div class="t-val">{ai["samples"]:,}</div></div>'
                f'</div></div>', unsafe_allow_html=True)
    ts=datetime.now().strftime('%Y%m%d_%H%M%S')
    c1,c2=st.columns(2)
    with c1: st.download_button("⬇️  Download JSON",
    ...
    with c2: st.download_button("📄  Download PDF",data=generate_pdf_bytes(result),
        file_name=f"instrunet_{ts}.pdf",mime="application/pdf",use_container_width=True)

def render_viz(result):
    st.markdown(f'<div class="card"><h3>〰️ Audio Visualizations</h3>'
...
def render_app(dark: bool):
    ...
    # Header
    st.markdown('<div class="hdr"><h1>🎵 InstruNet <span>AI</span></h1>'
    ...
    with cu:
        ...
        st.markdown(f'<div style="padding:.5rem 2.5rem;font-size:.79rem;color:{muted};'
                    ...
                    f'👤 Signed in as <strong style="color:{text}">{name}</strong></div>',
                    unsafe_allow_html=True)
    with ct:
    ...
    with clo:
        if st.button("Sign Out", key="signout_btn"):
            ...
            # Clear streamlit-authenticator session keys
    ...
    af = st.file_uploader("Drop audio file here or click to browse — WAV · MP3 · OGG · First 3 seconds analyzed",
    ...
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<div class="ftr">© 2026 InstruNet AI &nbsp;·&nbsp; Multi-Task CNN · Instrument · Quality · Condition</div>',
                unsafe_allow_html=True)
...
def main():
    ...
    # v0.4.x sets authentication_status in session_state via cookie automatically
